...
<template>
  <div class="w-full bg-quinary p-10">
    ...
    <div class="mb-6">
      ...
      <div class="flex items-center gap-4 justify-between">
        <div v-if="editMode.travail">
          <input 
            type="text"
            v-model="edited.travail"
            :placeholder="placeholderTravail"
    ...
    <RouterLink to="/account">
      <Typography variant="h2" component="h2" font="scholar" weight="regular" theme="gold" class="text-center pt-5">Retour au compte</Typography>
    </RouterLink>
    ...
    <div class="mb-6" v-for="(adresse, index) in adresses" :key="index">
      ...
      <div class="flex items-center gap-4 justify-between">
        ...
        <div class="flex flex-col gap-2 items-end">
          ...
          <Button variant="secondary" @click="editAdresse(index)">
            {{ editMode.adresses[index] ? 'OK' : 'Modifier' }}
          </Button>
        </div>
      </div>
    </div>

    <div class="flex justify-center align-center pt-20">
...
const editAdresse = (index) => {
  ...
  if (editMode.value.adresses[index]) {
    localStorage.setItem('userAdresses', JSON.stringify(adresses.value))
  }
  editMode.value.adresses[index] = !editMode.value.adresses[index]
}
